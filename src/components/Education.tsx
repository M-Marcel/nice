...
const Education = ({ portfolioData, updatePortfolioData, setActiveModal, setEducationToEdit, isPublished }: EducationProps) => {
    ...
    // Get initial educations - only from localStorage, never from backend initially
    ...
    useEffect(() => {
        ...
        window.addEventListener('educationUpdate', handleEducationUpdate as EventListener);
        return () => {
            window.removeEventListener('educationUpdate', handleEducationUpdate as EventListener);
        };
    }, [editingIndex]);
